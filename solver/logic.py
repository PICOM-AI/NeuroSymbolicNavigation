...
class Logic:
            ...
            match sym.name:
                case "action_taken":
                    if len(sym.arguments) == 4:
                        while len(self.robot) < sym.arguments[1].number + 1:
                            self.robot.append(0)
                        self.robot[sym.arguments[1].number] = sym.arguments[
                            0].number

    def setup(self):
        self.max_obstacles = min(pow((self.radius * 2 + 1), 2) - 1,
                             len(self.instance.obstacles))
        self.ctl = clingo.Control(["-c", f"horizon={self.horizon}", "-c",
                                   f"obstacles={self.max_obstacles}", "-c",
    ...
    def reset_clingo_externals(self):
        ...
        for i in range(1, self.instance.size + 1):
            for j in range(1, self.instance.size + 1):
                self.ctl.assign_external(Function("robot", [
                    Number(i),
                    Number(j)
                ]), False)
        ...
        for i in range(self.radius + 1):
            ...
            for j in range(self.radius + 1):
                self.ctl.assign_external(Function("restricted_area", [
                    Number(i),
                    Number(j)
                ]), False)
                self.ctl.assign_external(Function("restricted_area", [
    ...
    def set_clingo_externals(self):
        midpoint = self.instance.robot

        # robot
        self.ctl.assign_external(
            Function("robot", [Number(midpoint[0]),
        ...
        for i in range(self.radius + 1):
            for j in range(self.radius + 1):
                ...
                if absolut in self.instance.restricted_areas and absolut not in self.instance.entered_restricted_areas:
                    self.ctl.assign_external(
                        Function("restricted_area", [Number(i), Number(j)]), True)
        ...
        for c, f in enumerate(self.instance.obstacles):
            ...
            if abs(relative[0]) > self.radius or abs(
                    relative[
                        1]) > self.radius or f in self.instance.hit_obstacles:
                ...
                self.ctl.assign_external(
                    Function("fcol",
                             [Number(count_obstacle), Number(relative[0]),
                              Number(0)]),
                    True)
                self.ctl.assign_external(
                    Function("frow",
                             [Number(count_obstacle), Number(relative[1]),
                              Number(0)]),
                    True)
                count_obstacle += 1
        for i in range(count_obstacle, self.max_obstacles):
    ...
    def get_action(self, cache):
        ...
        if self.instance.visited[self.instance.robot] > 10:
            self.cache.clear()
        ...
        # Externals include cell information (walls, obstacles, restricted_areas) as well as
